...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Keep these declared early to avoid TDZ issues for callers.
    let player = null;
    let shouldAutoPlay = false;  // Auto-play flag

    let _ytReadyResolve = null;
    const ytReady = new Promise((resolve) => { _ytReadyResolve = resolve; });
    let _pendingMusicQuery = null;

    function extractMusicQuery(textRaw) {
        const text = (textRaw || "").trim();
        if (!text) return null;
        const lower = text.toLowerCase();

        // Patterns that mean "play music" even without a title
        const genericMusicPhrases = [
            "turn on some music",
            "turn on music",
            "play some music",
            "play music",
        ...
        if (genericMusicPhrases.some(p => lower === p)) return "__DEFAULT__";

        // Commands with optional query after them
        const prefixes = [
            "включи музыку",
            "включи песню",
            "включи",
        ...
        const query = text.substring(matched.length).trim();
    ...
    function onPlayerReady(event) {
        ...
        if (typeof _ytReadyResolve === "function") {
            _ytReadyResolve(true);
    ...
    async function updateAccountsList() {
        try {
            const resp = await fetch('/api/gmail/accounts');
            const data = await resp.json();
            ...
            otherAccounts.forEach(email => {
                ...
                accountItem.addEventListener('click', () => {
                    ...
                    // Switch backend session + reload inbox for that account
                    (async () => {
                        try {
                            await fetch('/api/gmail/switch', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/json' },
                                body: JSON.stringify({ email })
                            });
                            // Load cached emails for the switched account (skip background sync here)
                            await loadEmails('fast', true);
                            // Then do a background full sync to populate/update cache for that account
                            localStorage.removeItem('lastFullEmailSync');
                            await syncEmailsBackground('full');
                        } catch (e) {
                            console.error('Switch Gmail account error:', e);
                        }
                    })();
                });
    ...
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (resp.ok) {
                ...
                closeModal();
                // Make manual login behave like "Login with Gmail" if this email has Gmail connected
                try {
                    const sw = await fetch('/api/gmail/switch', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ email })
                    });
                    if (sw.ok) {
                        localStorage.setItem(GMAIL_AUTH_KEY, JSON.stringify({ authenticated: true, email }));
                        localStorage.removeItem('lastFullEmailSync'); // force refresh for this account
                        if (gmailSection) gmailSection.style.display = 'block';
                        await loadEmails('fast', true);
                        setTimeout(() => loadEmails('full').catch(() => {}), 600);
                    } else {
                        // Not a connected Gmail account — show connect prompt
                        localStorage.removeItem(GMAIL_AUTH_KEY);
                        if (gmailSection) gmailSection.style.display = 'none';
                        loadEmails();
                    }
                } catch (e) {
                    console.error('Manual login Gmail attach error:', e);
                    localStorage.removeItem(GMAIL_AUTH_KEY);
                    if (gmailSection) gmailSection.style.display = 'none';
                    loadEmails();
                }
    ...
    if (gmailDisconnectBtn) {
        gmailDisconnectBtn.addEventListener('click', async () => {
            try {
                // Clear cached inbox for current account before logging out
                try { await fetch('/api/emails/clear-cache', { method: 'POST' }); } catch (e) { /* ignore */ }
    ...
    logoutBtn.addEventListener('click', async () => {
        try {
            ...
            // Also log out from Gmail + clear cached inbox so UI becomes empty immediately
            try { await fetch('/api/emails/clear-cache', { method: 'POST' }); } catch (e) { /* ignore */ }
            try { await fetch('/api/gmail/logout', { method: 'POST' }); } catch (e) { /* ignore */ }
            localStorage.removeItem(SESSION_TOKEN_KEY);
            localStorage.removeItem(CURRENT_ACCOUNT_KEY);
            localStorage.removeItem(GMAIL_AUTH_KEY);
            localStorage.removeItem('lastFullEmailSync');
            ...
            // Clear email list immediately
            try { renderEmailConnectPrompt(); } catch (e) { /* ignore */ }
        } catch (e) {
